  ...
  return (
    <header className="sticky top-0 z-50 flex h-16 items-center justify-between bg-neutral-950 px-4 border-b border-neutral-800">
      <div className="flex items-center gap-4">
        <Link href="/" className="flex items-center gap-3 group relative" onClick={() => analytics.navigation({ target: 'home', type: 'main' })}>
      ...
      <div className="flex items-center gap-2">
        {isLoggedIn ? (
          <button type="button" className="p-2 hover:bg-neutral-800 rounded-full transition-colors">
            <div className="w-8 h-8 rounded-full bg-lime-400/10 flex items-center justify-center border border-lime-400/20">
              <User className="w-5 h-5 text-lime-400" />
            </div>
          </button>
        ) : (
          <button type="button" className="flex items-center gap-2 px-2 sm:px-4 py-1.5 text-sm font-medium text-lime-400 border border-neutral-800 rounded-full hover:bg-lime-400/10 hover:border-lime-400/30 transition-colors" onClick={() => analytics.authClick('sign_in')}>
